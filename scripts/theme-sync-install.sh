...
main() {
  ...
  if grep -q '^\s*themes:\s*$' "$XUZ_CONFIG"; then
    # Replace the existing themes block. We find "themes:" line and the
    # next line that starts a new top-level key (no leading whitespace).
    local tmp
    tmp=$(awk -v block="$theme_block" '
      BEGIN { found = 0; skip = 0 }
      /^[^ \t]/ && !skip { skip = 0 }
      !skip { print }
      /^themes:/ { found = 1; print block; skip = 1; next }
      /^[^ \t]/ && found { skip = 0 }
    ' "$XUZ_CONFIG")
    echo "$tmp" > "$XUZ_CONFIG"
  else
  ...
  if grep -q '^theme:\s' "$XUZ_CONFIG"; then
    tmp=$(sed "s|^theme:\s.*|theme: ${slug}|" "$XUZ_CONFIG")
    echo "$tmp" > "$XUZ_CONFIG"
  else
