#!/usr/bin/env python3
"""Test: multiline command with YAML block scalars (| and |-)."""
import fcntl
import os
import pty
import re
...
# Block scalar with | (keeps trailing newline)
CONFIG_PIPE = """\
theme: default
aliases:
  llama:
...
# Block scalar with |- (strips trailing newline)
CONFIG_PIPE_DASH = """\
theme: default
aliases:
  llama:
...
def run_tui(binary: str, config: str, label: str):
...
def main():
    ...
    run_tui(binary, CONFIG_PIPE, "BLOCK SCALAR | (keeps trailing newline)")
    run_tui(binary, CONFIG_PIPE_DASH, "BLOCK SCALAR |- (strips trailing newline)")
